...
# - - - Registration - - - - - - - - - - - - - - - - - - - -
    ...
    def getConferenceSessions(self, request):
        """Given a conference, return all child sessions."""
    ...
    def getConferenceSessionsByType(self, request):
        """Given a conference and a type, return all child sessions."""
        conference_key = ndb.Key(urlsafe=request.websafeConferenceKey)
        sessions = Session.query(Session.typeOfSession == request.typeOfSession, ancestor=conference_key)
        return SessionForms(
            items=[self._copySessionToForm(session) for session in sessions],
        )
    ...
    def createSession(self, request):
        ...
        if not user:
            raise endpoints.UnauthorizedException('Authorization required')
        # ensure that the user is the organizer of the conference
        conference_key = ndb.Key(urlsafe=request.websafeConferenceKey)
        # if user != conference_key.parent().get():
        #     print user, conference_key, conference_key.parent().get()
        #     raise endpoints.UnauthorizedException('User must be conference organizer.')

        if not all((request.name, request.date, request.startTime, request.typeOfSession)):
            raise endpoints.BadRequestException("Required fields missing.")
